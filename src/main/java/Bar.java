...
public class Bar {
	...
	private Color color;
	private int textYOffset; // Thêm biến để xác định vị trí vẽ giá trị
	...
	public Bar(int x, int y, int width, int value, Color color) {
		...
		// Xác định vị trí dọc để vẽ giá trị (ở giữa thanh)
		this.textYOffset = y - value / 2;
	}
	...
	public void draw(Graphics g) {
		...
		// Điều chỉnh textYOffset để căn giữa văn bản
		// int textY = textYOffset + (g.getFontMetrics().getHeight()/2 );
	...
	public void clear(Graphics g) {
		...
		g.setColor(ColorManager.CANVAS_BACKGROUND); // Màu chữ có thể điều chỉnh
		String valueString = Integer.toString(value);
		int textWidth = g.getFontMetrics().stringWidth(valueString);
		int textX = x + (width - textWidth) / 2;
